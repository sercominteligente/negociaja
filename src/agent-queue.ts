...
export async function enqueueAgentResponse(env:Env,tenantId:string,conversationId:string,inputMessageId:string){
  ...
  const jobId=makeId('ajob');await env.DB.prepare(`INSERT INTO agent_async_jobs (id,tenant_id,conversation_id,input_message_id,status) VALUES (?,?,?,?,'pending')`).bind(jobId,tenantId,conversationId,inputMessageId).run();
  try{await (env.AGENT_QUEUE as Queue<AgentQueueJob>).send({job_id:jobId,tenant_id:tenantId,conversation_id:conversationId,input_message_id:inputMessageId});return{queued:true,job_id:jobId};}
  catch(error){const msg=error instanceof Error?error.message:String(error);await env.DB.prepare(`UPDATE agent_async_jobs SET status='enqueue_failed',last_error=?,updated_at=datetime('now') WHERE id=?`).bind(msg,jobId).run();throw error;}
}
...
export async function consumeAgentQueue(batch:MessageBatch<AgentQueueJob>,env:Env){
  for(const message of batch.messages){const job=message.body;try{
    const receipt=await env.DB.prepare(`SELECT status,attempts FROM agent_async_jobs WHERE id=? AND tenant_id=? AND input_message_id=? LIMIT 1`).bind(job.job_id,job.tenant_id,job.input_message_id).first<{status:string;attempts:number}>();
    if(!receipt||receipt.status==='completed'){message.ack();continue;}
    const claimed=await env.DB.prepare(`UPDATE agent_async_jobs SET status='processing',attempts=attempts+1,started_at=COALESCE(started_at,datetime('now')),updated_at=datetime('now') WHERE id=? AND tenant_id=? AND status IN ('pending','retry','enqueue_failed')`).bind(job.job_id,job.tenant_id).run();
    if(!claimed.meta.changes&&receipt.status==='processing'){message.retry();continue;}
