...
export default{async fetch(request:Request,env:Env):Promise<Response>{const url=new URL(request.url),method=request.method.toUpperCase();if(url.pathname.startsWith('/api/')&&!SAFE.has(method)&&!machineRoute(url.pathname)&&!validOrigin(request))return json({error:'Origem não autorizada.',code:'invalid_origin'},403);return stable.fetch(request,env)}};
