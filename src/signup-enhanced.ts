...
export async function handleEnhancedSignup(request:Request,env:Env,url:URL):Promise<Response|null>{
  ...
  try{
    ...
    const settingsInsert=hasSegmentLabel
      ...
      : env.DB.prepare(`INSERT INTO tenant_settings (tenant_id,public_name) VALUES (?,?)`).bind(tenantId,companyName);
    await env.DB.batch([
    ...
    const delivery=await deliver(env,tenantId,email,name,`${url.origin}/confirmar-email?token=${encodeURIComponent(rawToken)}`);
    await audit(env,null,tenantId,'auth.signup.created','user',userId,{email,segment:coreSegment,segment_label:segmentLabel,segment_label_persisted:hasSegmentLabel,email_sent:delivery.sent});
    return json({data:{ok:true,tenant_slug:slug,email,email_sent:delivery.sent,segment:coreSegment,segment_label:segmentLabel,message:'Cadastro criado. Confirme seu e-mail para ativar a conta.'}},201);
  }catch(error){
    console.error(JSON.stringify({event:'signup_failed',email_domain:email.split('@')[1]||null,segment:requested,error:error instanceof Error?error.message:String(error)}));
    return json({error:'Não foi possível concluir o cadastro agora. A configuração do banco está sendo atualizada; tente novamente em alguns instantes.'},503);
  }
}
