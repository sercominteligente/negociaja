...
export interface Env { DB: D1Database; FILES: R2Bucket; ASSETS: Fetcher; AGENT_QUEUE?: Queue<unknown>; HML_BOOTSTRAP_TOKEN?: string; RESEND_API_KEY?: string; EMAIL_FROM?: string; CREDENTIALS_KEY?: string; OPENAI_API_KEY?: string; OPENAI_TRANSCRIBE_MODEL?: string; OPENAI_VISION_MODEL?: string; OPENAI_AGENT_MODEL?: string; }
...
export function escapeHtml(value:unknown){return String(value??'').replace(/[&<>'"]/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch]||ch));}
...
export async function authenticate(request:Request,env:Env):Promise<Actor|null>{
  ...
  const row=await env.DB.prepare(`SELECT s.id session_id,s.user_id,s.platform_user_id,s.tenant_id,s.role,u.name user_name,u.email user_email,u.status user_status,p.name platform_name,p.email platform_email,p.status platform_status,t.status tenant_status FROM auth_sessions s LEFT JOIN users u ON u.id=s.user_id LEFT JOIN platform_users p ON p.id=s.platform_user_id LEFT JOIN tenants t ON t.id=s.tenant_id WHERE s.token_hash=? AND s.revoked_at IS NULL AND datetime(s.expires_at)>datetime('now') LIMIT 1`).bind(tokenHash).first<{session_id:string;user_id:string|null;platform_user_id:string|null;tenant_id:string|null;role:Role;user_name:string|null;user_email:string|null;user_status:string|null;platform_name:string|null;platform_email:string|null;platform_status:string|null;tenant_status:string|null}>();
  if(!row)return null;
  if(row.user_id&&(row.user_status!=='active'||!['trial','active'].includes(row.tenant_status||'')))return null;
  ...
  return{sessionId:row.session_id,actorId:row.user_id||row.platform_user_id||'',actorType:row.user_id?'tenant_user':'platform_user',role:row.role,tenantId:row.tenant_id,name:row.user_name||row.platform_name||'Usuário',email:row.user_email||row.platform_email||''};
}
