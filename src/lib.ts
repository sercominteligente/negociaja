...
export async function authenticate(request:Request,env:Env):Promise<Actor|null>{
  ...
  const row=await env.DB.prepare(`SELECT s.id session_id,s.user_id,s.platform_user_id,s.tenant_id,s.role,u.name user_name,u.email user_email,u.status user_status,p.name platform_name,p.email platform_email,p.status platform_status,t.status tenant_status,sub.status subscription_status,sub.trial_ends_at,sub.current_period_end,pl.grace_days FROM auth_sessions s LEFT JOIN users u ON u.id=s.user_id LEFT JOIN platform_users p ON p.id=s.platform_user_id LEFT JOIN tenants t ON t.id=s.tenant_id LEFT JOIN tenant_subscriptions sub ON sub.tenant_id=t.id LEFT JOIN platform_plans pl ON pl.id=sub.plan_id WHERE s.token_hash=? AND s.revoked_at IS NULL AND datetime(s.expires_at)>datetime('now') LIMIT 1`).bind(tokenHash).first<{session_id:string;user_id:string|null;platform_user_id:string|null;tenant_id:string|null;role:Role;user_name:string|null;user_email:string|null;user_status:string|null;platform_name:string|null;platform_email:string|null;platform_status:string|null;tenant_status:string|null;subscription_status:string|null;trial_ends_at:string|null;current_period_end:string|null;grace_days:number|null}>();
  if(!row)return null;
  if(row.user_id){
    if(row.user_status!=='active'||!['trial','active'].includes(row.tenant_status||''))return null;
    if(await expireTenantIfNeeded(env,row))return null;
  }
