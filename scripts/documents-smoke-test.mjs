...
console.log('HML document smoke testing');
...
for(const [type,prefix] of [['quote','ORC-'],['order','PED-'],['receipt','REC-'],['invoice','FAT-']]){
  ...
  const view=await fetch(`${base}${out.p.data.view_url}`,{headers:{authorization}});
  const html=await view.text();
  assert(view.status===200,`${type} view returned ${view.status}`);
  assert((view.headers.get('content-type')||'').includes('text/html'),`${type} view is not HTML`);
  assert(html.includes(out.p.data.document_number),`${type} snapshot view missing document number`);
  assert(html.includes('Cliente Documento CI'),`${type} snapshot view missing customer`);
  issued.push(out.p.data.document_number);
}

const docs=await json('/api/ops/documents');
assert(docs.r.ok,`document list returned ${docs.r.status}`);
assert(Array.isArray(docs.p.data),'document list must be array');
for(const number of issued)assert(docs.p.data.some((d)=>d.document_number===number),`document ${number} missing from list`);

console.log('HML document issuance smoke test passed.');
